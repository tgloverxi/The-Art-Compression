...
public:
   ...
   * This function will call helper function buildTree.
    ...
    */
   PNG render();
//    void render(PNG & img,Node*n);
//    void putImage(PNG & toPutImg, PNG & oriImg, pair<int,int> p, int k);
//    vector<pair<int,int>> getSplitCoord(pair<int,int> SE, int k);
//    pair<int,int> getCoordlr(pair<int,int>p, int k);
//    pair<int,int> fixCoord(int x, int y, int k);
//    pair<int,int>findPixel();
    PNG buildPNG(Node*n);
    PNG combine(PNG p1,PNG p2,PNG p3,PNG p4,pair<int,int> center);
//
   /*
   ...
   * You may want a recursive helper function for this one.
   ...
   void prune(double tol);
    void prune(Node* nn, double tol);

    /* returns the number of nodes in the current toqutree. primarily used
     * for debugging and testing.
     */
   
    bool  withinTol(Node* n,HSLAPixel p,double tol);
...
private:
   ...
   */
   Node * buildTree(PNG * im, int k);
    double getEntropy(stats* data,pair<int,int> ul, pair<int,int> lr);
//    pair<int,int> getCentre();
    PNG* getChild(pair<int,int>sp,int k,PNG* im);
